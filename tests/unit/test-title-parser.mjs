...
check('奉魔|官方中文|Build.24618003+全DLC|解压即撸|', parseGameTitle('奉魔|官方中文|Build.24618003+全DLC|解压即撸|'), [
  '奉魔'
]);
...
check(
  '候选不含抢先试玩',
  huanshi.some((t) => t.includes('抢先') || t.includes('试玩')),
  false
);
...
check(
  '汇总贴候选不含汇总贴噪声',
  digest.some((t) => t.includes('汇总贴') || t.includes('顶置')),
  false
);
...
check(
  '候选不含 DLC 名（初心请鞭）',
  huaxia.some((t) => t.includes('初心请鞭') || t.includes('英勇逐鹿')),
  false
);
check(
  '候选不含"者"残留',
  huaxia.some((t) => t.includes('者')),
  false
);
...
check(
  'Switch520.com 被噪声清洗',
  spiritOfNorth.some((t) => t.includes('Switch520') || t.includes('520.com') || t.includes('.com')),
  false
);
