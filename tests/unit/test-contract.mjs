...
check(
  '合法 view_detail',
  validateMessage('TRACK_EVENT', { data: { type: 'view_detail', gameName: '游戏A' } }).ok,
  true
);
check(
  '合法 view_list（无 gameName）',
  validateMessage('TRACK_EVENT', { data: { type: 'view_list', itemCount: 5 } }).ok,
  true
);
check(
  '合法 steam_tags_update（keywords 数组）',
...
check(
  'gameName 超 200 拒绝',
  validateMessage('TRACK_EVENT', { data: { type: 'click_detail', gameName: 'x'.repeat(201) } }).ok,
  false
);
check(
  'keywords 非数组拒绝',
...
check(
  'SAVE_MANUAL_MAPPING 非数字 appId 拒绝',
  validateMessage('SAVE_MANUAL_MAPPING', { gameName: '游戏', appId: '12x' }).ok,
  false
);
...
check(
  'GET_RECOMMENDATIONS 合法',
  validateMessage('GET_RECOMMENDATIONS', { games: [{ name: '游戏A', url: 'x', appId: 1 }] }).ok,
  true
);
...
check(
  'GET_GAME_CACHE_LIST 合法',
  validateMessage('GET_GAME_CACHE_LIST', { keyword: 'RPG', minRating: 70, page: 1 }).ok,
  true
);
...
check(
  'SEARCH_DOWNLOAD_SITES 合法',
  validateMessage('SEARCH_DOWNLOAD_SITES', { gameName: '游戏', appId: 123 }).ok,
  true
);
check('SEARCH_DOWNLOAD_SITES 缺名拒绝', validateMessage('SEARCH_DOWNLOAD_SITES', {}).ok, false);
check(
  'RECORD_DOWNLOAD_URLS_BATCH 合法',
  validateMessage('RECORD_DOWNLOAD_URLS_BATCH', { data: { domain: 'x', entries: [{ appId: 123, url: 'https://x' }] } })
    .ok,
  true
);
check(
  'RECORD_DOWNLOAD_URLS_BATCH 空 entries 合法',
  validateMessage('RECORD_DOWNLOAD_URLS_BATCH', { data: { entries: [] } }).ok,
  true
);
...
check(
  'RESTORE_BACKUP 带 moduleKeys 合法',
  validateMessage('RESTORE_BACKUP', { backupId: 'b-1', moduleKeys: ['settings'] }).ok,
  true
);
