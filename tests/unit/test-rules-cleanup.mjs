...
check(
  'domains 为空拒绝',
  rulesMod.validateAdapterRules({ version: 1, sites: [{ key: 'x', name: 'X', domains: [] }] }).ok,
  false
);
check(
  '类型错误拒绝（searchUrl 数字）',
...
check(
  'settings 密钥剔除（apiKey 清空）',
  rulesMod.sanitizeImportedModule('settings', { llmConfig: { apiKey: 'sk-secret' } }).llmConfig.apiKey,
  ''
);
check(
  'adapterRules 走规则校验（非法返回 null）',
  rulesMod.sanitizeImportedModule('adapterRules', { version: 1, sites: [{ key: 'x' }] }),
  null
);
check('adapterRules 合法通过', rulesMod.sanitizeImportedModule('adapterRules', validRules) !== null, true);
check(
  '未知模块非纯 JSON 拒绝',
  rulesMod.sanitizeImportedModule('steamCache', () => 1),
  null
);
check(
  '未知模块纯 JSON 通过',
  JSON.stringify(rulesMod.sanitizeImportedModule('steamCache', { a: 1 })),
  JSON.stringify({ a: 1 })
);
