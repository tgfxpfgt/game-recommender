 ...
 * 评分状态机函数（applyRatingsResponse/finishRatings）经 list._internal。
 * Batch scheduling split from list-page.js (v5.1.0); scheduler state lives in
 * list._state, rating-application functions via list._internal.
 */
import * as list from './list-page.js';
...
function ensureNameToImage(name) {
    const batchState = list._state.batchState;
    ...
    batchState.nameToImage[name] = item
      ? builder.isImageAppIdEnabled()
        ? builder.extractSteamImageInfo(item.element)
        : null
      : null;
...
async function fireBatch(names) {
    ...
    try {
      ...
      list._internal.applyRatingsResponse(ratings, 'first');
      const job = list._state.ratingsJob;
      ...
      if (pendingCount > 0) {
        ...
        list._internal.scheduleFallbacks();
      } else {
...
function applyRecommendationResults(results) {
    ...
    for (const result of results) {
      ...
      badges.prependRecBadge(item, result.recommendation, settings);
      if (recEnabled && result.recommendation.score >= threshold) {
        badges.highlightItem(item);
        highlighted++;
      }
    }
    ...
    batchState.observer = new MutationObserver((mutations) => {
      ...
      scanTimer = setTimeout(() => {
        ...
        for (const node of nodes) {
          const found = builder.findItemsInContainer(node);
...
export function requestSteamRatings(items, settings) {
    if (!items || items.length === 0) return;
    initBatchState(settings);
    enqueueItems(items);
    // 名字全集来自 processItems（nameToImage 已惰性化）
    const jobNames = list._state.batchState.processItems.map((i) => i.name).filter((n) => n && n.length > 1);
    ...
    status.showStatus(
      '正在获取 Steam 好评率',
      list._state.batchState.processItems.length,
      '缓存优先检索中...'
    );
    list._internal.createRatingsJob(list._state.batchState.processItems, settings, jobNames);
    maybeFetchNextBatch();
    startListScan();
}
...
export async function requestRecommendations(items, settings, nameToImage) {
    const batchState = list._state.batchState;
