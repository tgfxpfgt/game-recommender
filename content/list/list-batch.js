 ...
 * 状态（ratingsJob/batchState）存于 _state（唯一状态容器），
 * 评分状态机函数（applyRatingsResponse/finishRatings）经 _internal。
...
async function fireBatch(names) {
  ...
  try {
    ...
    _internal.applyRatingsResponse(ratings, 'first');
    ...
    if (pendingCount > 0) {
      ...
      if (!fired && !job.finished) _internal.finishRatings();
    }
  } catch (e) {
    dbg('Steam好评率检索失败: ' + String(e));
    batchState.inflight = false;
    _internal.finishRatings();
  }
}
...
export function requestSteamRatings(items, settings) {
  ...
  const jobNames = _state.batchState.processItems.map((i) => i.name).filter((n) => n && n.length > 1);
